...
export class DesktopWindowManager {
      ...
      return [
        {
          ...
          click: () => void this.actions.connectCloudAccount()
            .catch((error) => this.actions.showError('Could not connect CloudCLI Cloud account', error)),
        },
      ];
    }
  ...
  buildAppMenu() {
    ...
    const cloudAccountLabel = cloudState.account?.apiKey
      ? (cloudState.account?.email ? `Connected: ${cloudState.account.email}` : 'CloudCLI Cloud Connected')
      : (cloudState.account?.email ? `Reconnect: ${cloudState.account.email}` : 'Connect CloudCLI Cloud Account...');
    ...
    const template = [
      ...
      },
      {
        ...
        submenu: [
          ...
          { type: 'separator' },
          {
            label: 'Open Local WindCli',
            accelerator: 'CmdOrCtrl+L',
            click: () => void this.actions.openLocalInDesktop().catch((error) => this.actions.showError('Could not open local WindCli', error)),
          },
          {
        ...
        submenu: [
          {
            ...
            click: () => void this.actions.connectCloudAccount().catch((error) => this.actions.showError('Could not connect CloudCLI Cloud account', error)),
          },
          {
            label: 'Refresh Cloud Environments',
            click: () => void this.actions.refreshCloudEnvironments().catch((error) => this.actions.showError('Could not load CloudCLI Cloud environments', error)),
            enabled: Boolean(cloudState.account?.apiKey),
          },
          {
            label: 'Logout WindCli Account',
    ...
    const template = [
      {
        ...
        submenu: [
          {
            label: localState.localServerRunning ? 'Open Local in WindCli' : 'Start Local in WindCli',
            click: () => void this.actions.openLocalInDesktop().catch((error) => this.actions.showError('Could not open local WindCli', error)),
          },
          {
