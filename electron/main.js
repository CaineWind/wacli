...
async function copyDiagnostics() {
  ...
  await dialog.showMessageBox(desktopWindow?.getMainWindow() || undefined, {
    ...
    message: 'WindCli desktop diagnostics were copied to the clipboard.',
  });
}
...
async function refreshCloudEnvironments({ showErrors = false } = {}) {
  ...
  try {
    ...
    if (authState === 'expired') {
      const expiredError = new Error('Your CloudCLI Cloud session expired. Reconnect your account.');
      if (showErrors) {
        await showError('CloudCLI Cloud login required', expiredError);
        return [];
      }
      throw expiredError;
    }
    if (showErrors) {
      await showError('Could not load CloudCLI Cloud environments', error);
      return [];
    }
    throw error;
  } finally {
...
async function handleDeepLink(url) {
  ...
  if (!pendingCloudConnectStartedAt || Date.now() - pendingCloudConnectStartedAt > AUTH_CALLBACK_TTL_MS) {
    await showError('CloudCLI Cloud account connection failed', new Error('No recent CloudCLI Cloud account connection was started from this app.'));
    return;
  }

  const apiKey = parsed.searchParams.get('api_key');
  if (!apiKey) {
    await showError('CloudCLI Cloud account connection failed', new Error('The callback did not include an API key.'));
    return;
  }
  ...
  dialog.showMessageBox(desktopWindow?.getMainWindow() || undefined, {
    type: 'info',
    title: 'CloudCLI Cloud account connected',
    message: cloud.getAccount()?.email ? `Connected as ${cloud.getAccount().email}.` : 'CloudCLI Cloud account connected.',
  }).catch(() => {});
}
...
async function copyLocalWebUrl() {
  ...
  if (!shareableUrl) {
    throw new Error('Local WindCli URL is not available yet.');
  }
  ...
  await dialog.showMessageBox(desktopWindow?.getMainWindow() || undefined, {
    ...
    detail: isLanUrl
      ...
      : `${shareableUrl}\n\nThis URL works on this computer. Enable LAN access before starting Local WindCli to copy a phone-accessible URL.`,
  });
...
async function updateDesktopSetting(key, value) {
  ...
  if (result.requiresRestartNotice) {
    await dialog.showMessageBox(desktopWindow?.getMainWindow() || undefined, {
      ...
      detail: 'Quit WindCli and stop the local server, then open Local WindCli again.',
    });
  }
...
async function showEnvironmentPicker() {
  ...
  const choices = ['Local WindCli', ...environments.map((environment) => {
  ...
  const response = await dialog.showMessageBox(desktopWindow?.getMainWindow(), {
    ...
    title: 'Switch WindCli Environment',
...
async function openEnvironmentInDesktop(environment) {
  ...
  if (environment.status !== 'running') {
    const response = await dialog.showMessageBox(desktopWindow?.getMainWindow(), {
      ...
      detail: 'WindCli can start it before opening the remote app.',
    });
...
function getRemoteEnvironmentMenuItems() {
  ...
  if (!cloudAccount?.apiKey) {
    return [{ label: 'Connect CloudCLI Cloud Account...', click: () => void connectCloudAccount() }];
  }
...
if (registerSingleInstance()) {
  bootstrap().catch(async (error) => {
    await showError('WindCli failed to start', error);
    app.quit();
  });
}
