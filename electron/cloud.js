...
export class CloudController {
  ...
  async cloudApi(pathname, options = {}) {
    if (!this.cloudAccount?.apiKey) {
      throw new Error('Connect your CloudCLI Cloud account first.');
    }
    ...
    try {
      ...
      if (error?.name === 'AbortError') {
        throw new Error(`CloudCLI Cloud API request timed out after ${Math.round(CLOUD_API_TIMEOUT_MS / 1000)} seconds.`);
      }
      throw error;
    } finally {
    ...
    if (!response.ok) {
      ...
      throw new Error(body.error || `CloudCLI Cloud API request failed: ${response.status}`);
    }
