...
# Auto-start WindCli server in background if not already running.
# This script is sourced from ~/.bashrc on sandbox shell open.

if ! pgrep -f "server/index.js" > /dev/null 2>&1; then
  nohup wacli start --port 3001 > /tmp/cloudcli-ui.log 2>&1 &
  disown

  echo ""
  echo "  WindCli is starting on port 3001..."
  echo ""
