...
# Sandboxed coding agents with a web & mobile IDE (WindCli)

[Docker Sandbox](https://docs.docker.com/ai/sandboxes/) templates that add [WindCli](https://github.com/CaineWind/wacli) on top of Claude Code and Codex. You get a full web and mobile IDE accessible from any browser on any device.
...
npx wind-agent-cli@latest sandbox ~/my-project --agent codex
```
...
If you install WindCli globally (`npm install -g wind-agent-cli`), you can also use:

```bash
wacli sandbox ls
wacli sandbox start my-project    # Restart and re-launch web UI
wacli sandbox logs my-project     # View server logs
```
...
```bash
npx wind-agent-cli@latest sandbox ~/my-project --env SERVER_PORT=8080
```
...
Restart WindCli for changes to take effect:

```bash
sbx exec my-project bash -c 'pkill -f "server/index.js"'
sbx exec -d my-project wacli start --port 3001
```
...
WindCli auto-starts via `.bashrc` when using `sbx run`.
...
- [Documentation](https://github.com/CaineWind/wacli/blob/main/docs/README.md) — full configuration guide
- [GitHub](https://github.com/CaineWind/wacli) — source code and issues
