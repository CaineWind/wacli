...
<p align="center">
 <a href="https://github.com/CaineWind/wacli/issues">Bug 报告</a> · <a href="../CONTRIBUTING.md">贡献指南</a>
</p>
...
> CloudCLI Cloud 是上游项目的第三方服务，并非由 WindCli 项目运营。
...
- **插件系统** - 通过自定义选项卡、后端服务与集成扩展 WindCli。 [开始构建 →](https://github.com/cloudcli-ai/cloudcli-plugin-starter)
...
启动 WindCli，只需一行 `npx`（需要 Node.js v22+）：
...
更多配置选项、PM2、远程服务器设置等，请参阅 **[文档 →](https://github.com/CaineWind/wacli/blob/main/docs/README.md)**。
...
WindCli 是 CloudCLI Cloud 的开源 UI 层。你可以在本地机器上自托管它，也可以使用提供团队功能与深入集成的 CloudCLI Cloud。

| | WindCli（自托管） | CloudCLI Cloud |
|---|---|---|
| **适合对象** | 需要为本地代理会话提供完整 UI 的开发者 | 需要部署在云端，随时从任何地方访问代理的团队与开发者 |
| **访问方式** | 通过 `[yourip]:port` 在浏览器中访问 | 浏览器、任意 IDE、REST API、n8n |
| **设置** | `npx wind-agent-cli` | 无需设置 |
| **机器需保持开机吗** | 是 | 否 |
...
> 两种方式都使用你自己的 AI 订阅（Claude、Cursor 等）— WindCli 提供环境，而非 AI。
...
WindCli 配备插件系统，允许你添加带自定义前端 UI 和可选 Node.js 后端的选项卡。在 Settings > Plugins 中直接从 Git 仓库安装插件，或自行开发。
...
| **[PRISM CloudCLI](https://github.com/jakeefr/cloudcli-plugin-prism)** | 在 WindCli 中提供 Claude Code 会话智能分析，包括 token 消耗可视化 |
...
WindCli 与 CloudCLI Cloud 是对 Claude Code 的扩展，而非旁观 — MCP 服务器、权限、设置、会话与 Claude Code 完全一致。

- **覆盖全部会话** — WindCli 会自动扫描 `~/.claude` 文件夹中的每个会话。Remote Control 只暴露当前活动的会话。
- **设置统一** — 在 WindCli 中修改的 MCP、工具权限等设置会立即写入 Claude Code。
...
需要。WindCli 只提供环境。你仍需自行获取 Claude、Cursor 或 Codex 订阅。CloudCLI Cloud 从 €7/月起提供托管环境。

</details>

<details>
<summary>能在手机上使用 WindCli 吗？</summary>
...
会的。自托管模式下，WindCli 读取并写入 Claude Code 使用的 `~/.claude` 配置。通过 UI 添加的 MCP 服务器会立即在 Claude Code 中可见。
...
- **[文档](https://github.com/CaineWind/wacli/blob/main/docs/README.md)** — 安装、配置、功能与故障排除指南
- **[Discord](https://discord.gg/buxwujPNRE)** — 获取帮助并与社区交流
- **[GitHub Issues](https://github.com/CaineWind/wacli/issues)** — 报告 Bug 与建议功能
